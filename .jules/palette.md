...
## 2026-04-13 - Explicit empty state messages in filtered lists
**Learning:** Leaving lists empty when a filter is applied can confuse the user, making them believe there is a loading error instead of a search with no results.
**Action:** Always provide useful empty states that inform the user why the list is empty (e.g., 'No themes matching XYZ').
