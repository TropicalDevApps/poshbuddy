...
impl PluginInstaller {
    ...
    async fn install_module(&self, module_name: &str) -> Result<Option<String>, io::Error> {
        let output = tokio::process::Command::new("powershell")
            ...
            .args([
                "-Command",
                "Install-Module -Name $env:TARGET_MODULE -Scope CurrentUser -Force -AllowClobber -Confirm:$false; \
                 (Get-Module -ListAvailable -Name $env:TARGET_MODULE).Version | Select-Object -First 1",
            ])
            .output()
            .await?;
    ...
    fn check_module_installed(module_name: &str) -> Result<bool, io::Error> {
        let output = Command::new("pwsh")
            ...
            .args([
                "-Command",
                "Get-Module -ListAvailable -Name $env:TARGET_MODULE | Select-Object -First 1",
            ])
            .output()?;
    ...
    pub async fn uninstall_module(&self, module_name: &str) -> Result<(), io::Error> {
        let output = tokio::process::Command::new("powershell")
            ...
            .args([
                "-Command",
                "Uninstall-Module -Name $env:TARGET_MODULE -Scope CurrentUser -Force -Confirm:$false",
            ])
            .output()
            .await?;
    ...
    pub fn get_module_info(&self, module_name: &str) -> Result<Option<ModuleInfo>, io::Error> {
        let output = Command::new("pwsh")
            ...
            .args([
                "-Command",
                "Get-Module -ListAvailable -Name $env:TARGET_MODULE | Select-Object Name, Version, Description | Format-List",
            ])
            .output()?;
